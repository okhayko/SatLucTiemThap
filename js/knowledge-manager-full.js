...
async function exportCompleteBackup() {
    try {
        ...
        alert(`✅ 完整备份导出成功！\n\n📦 备份内容：\n` +
            ...
            `- 🎮 ACJT卡组：${stats.acjtHasData ? '✓ 已备份' : '✗ 无数据'}\n\n` +
            `💾 文件名：${filename}\n\n` +
            `💡 建议保存到安全的位置！`);
        ...
        try {
            ...
            if (backupData.acjtGameData) {
                ...
                if (backupData.acjtGameData.playerState) {
                    ...
                    console.log('[完整备份] ✓ ACJT玩家状态已恢复, 楼层:', backupData.acjtGameData.playerState.floor);
                }

                // 恢复卡组
                if (backupData.acjtGameData.cardDeck) {
                    localStorage.setItem('acjt_card_deck', JSON.stringify(backupData.acjtGameData.cardDeck));
                    console.log('[完整备份] ✓ ACJT卡组已恢复, 卡片数:', backupData.acjtGameData.cardDeck.length);
                }
            ...
            if (backupData.graphRAGData && window.graphRAGLite) {
                updateProgress('正在恢复GraphRAG语义网络...');
                ...
                console.log('[完整备份] ✓ GraphRAG语义网络已恢复:', entityCount, '个实体');
            }
            ...
            alert(`✅ 完整备份导入成功！\n\n` +
                ...
                `${importedItems.acjtGameData ? '✓' : '✗'} 🎮 ACJT卡组游戏数据（${stats.acjtFloor ? '楼层' + stats.acjtFloor : '无数据'}）\n\n` +
